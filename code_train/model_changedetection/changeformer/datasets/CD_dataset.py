...
def load_img_name_list(dataset_path):
    print('lllllllllllllllllllllll',dataset_path)
...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.asarray(Image.open(A_path))
        img_B = np.asarray(Image.open(B_path))
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label_transform == 'norm':
            ...
            # print(label.shape)
            # label_indices = np.unique(label, return_inverse=True)[1]
            # print(label_indices)
            # label_indices = torch.from_numpy(np.array(label_indices))
            
            # label = torch.nn.functional.one_hot(label_indices, 3)
