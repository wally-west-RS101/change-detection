...
def make_numpy_grid(tensor_data, pad_value=0,padding=0):
    ...
    # print('kk',vis.dtype)
    # print('kg',vis.shape)
    # print('vvvv',vis.shape[2])
#     if vis.shape[2] == 3:
#         img_4band = np.zeros((vis.shape[0], vis.shape[1], 4), dtype=vis.dtype)
#         img_4band[:, :, :3] = vis  # Copy dữ liệu từ 3 kênh ban đầu

# # Đặt giá trị alpha (độ trong suốt) cho toàn bộ hình ảnh là 255
#         img_4band[:, :, 3] = vis[:,:,2]
#         vis = img_4band
        
#     if vis.shape[2] == 1:
#         # print(1111111111111111111111)
#         vis = np.stack([vis, vis, vis, vis], axis=-1)
             
    if vis.shape[2] == 1:
        # print(1111111111111111111111)
        vis = np.stack([vis, vis, vis], axis=-1)
    # print('j',vis.dtype)
    # print('jjjjjjj',vis.shape)
    return vis
