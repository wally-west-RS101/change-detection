...
class CDTrainer():
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            current_model_dict =  self.net_G.state_dict()
            loaded_state_dict = torch.load(os.path.join(self.checkpoint_dir, ckpt_name), map_location=self.device)['model_G_state_dict']
            ...
            checkpoint = torch.load(self.args.pretrain,
                                    map_location=self.device)
            current_model_dict =  self.net_G.state_dict()
            loaded_state_dict = torch.load(self.args.pretrain, map_location=self.device)['model_G_state_dict']
            new_state_dict={k:v if v.size()==current_model_dict[k].size()  else  current_model_dict[k] for k,v in zip(current_model_dict.keys(), loaded_state_dict.values())}
    ...
    def _visualize_pred(self):
        # print(self.G_final_pred)
        pred = torch.argmax(self.G_final_pred, dim=1, keepdim=True)
        modified_matrix = torch.where(pred == 0, 0, torch.where(pred == 1, 255, 128))
        # print('prrrrrrr',pred)
        # print('kkkkkkkkk',pred.shape)
        # if pred.shape[0] ==1 :
        #     pred_vis = pred*125
        # if pred.shape[0] ==2:
        #     pred_vis = pred*125
        # pred_vis = pred * 255
        pred_vis = modified_matrix
        # print(pred_vis)
        return pred_vis
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.batch_id, 500) == 1:
            ...
            # print('hhhhhhhhhhhh',vis_input2.shape)
            # print(self._visualize_pred().shape)
            vis_pred = utils.make_numpy_grid(self._visualize_pred())
            # print('lllllllllllllll',vis_pred.shape)
    ...
    def train_models(self):
        ...
        # model_summary = summary(self.net_G, input_size=(2,8, 224, 224), col_names=["input_size", "output_size", "num_params"], verbose=0, depth=10)
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            # print('gggggggggggggggggggg',self.optimizer_G.param_groups[0].keys())
