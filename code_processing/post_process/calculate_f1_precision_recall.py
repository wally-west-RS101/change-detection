...
def cal_intersection(df,df1):
    ...
    df = df.dissolve()
    print('bbbbbbbbbbbbbbbbbbbb',len(df))
    intersected_data = gpd.overlay(df, df1, how='intersection')
    # singlepart_gdf = intersected_data.explode()

    return intersected_data

def cal_per_recall_f1(df,df1,df2):
    # cal_per_recall_f1(data_qaqc,data_ml,intersection)
    TP = len(df2)
    print('hhhhhhhhhhhhhhhhh',len(df))
    if len(df) > TP :
        FN = len(df) - TP
    else:
        FN =  TP - len(df)
...
if __name__ == "__main__":
    ...
    input_dir_shp_qaqc = '/home/skymap/data/Bahrain_change/allqc/BAOCAO_NEW_QT_30t5/qc'
    input_dir_ml = '/home/skymap/data/Bahrain_change/allqc/BAOCAO_NEW_QT_30t5/shpall'
    ...
    total_ml = []
    total_qc = []
    out_dir = '/home/skymap/data/Bahrain_change/allqc/BAOCAO_NEW_QT_30t5/'
    ...
    name_exel = 'Report_1819_new.xlsx'
